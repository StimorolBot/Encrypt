...
async def logout_user(response: Response, current_user: UserRead = Depends(UserManager.get_current_user)) -> str:
    response.delete_cookie(key="access_token")
    # сделать пользователя неактивным
    return f"Пользователь {current_user.user_name} вышел из системы"
